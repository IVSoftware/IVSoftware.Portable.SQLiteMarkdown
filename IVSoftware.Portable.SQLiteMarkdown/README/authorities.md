...
Changes of state occur, nearly all of the time, by user actions on a visible projection surface. The UI surface that interacts with a `MarkdownContext` typically presents a list, grid, or tree-style view. This element is referred to as the `ObservableNetProjection` and is intended for binding to the platform-specific control directly. The UI elements that can also include:
...
In most scenarios, changes to the UI initiated by a non UI thread are quite rare. 