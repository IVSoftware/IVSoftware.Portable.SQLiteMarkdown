...
namespace IVSoftware.Portable.SQLiteMarkdown.MSTest
{
    ...
    public class UnitTestSQLiteParser
    {
        ...
        private class ExpressionTestEntry
        {
            ...
            internal string Report(MarkdownContext context, ValidationState validationState)
            {
...
[
  {
    ...
    ""Properties"": ""{\r\n  \""FirstName\"": \""Tom\"",\r\n  \""LastName\"": \""Tester\"",\r\n  \""Tags\"": \""[c# .net maui][c# wpf][c# winforms]\""\r\n}""
                ...
                WHERE {nameof(SelfIndexedProfile.LikeTerm)} LIKE '%{expr}%'
                   OR {nameof(SelfIndexedProfile.ContainsTerm)} LIKE '%{expr}%'
                ...
                expected = @" 
                SELECT * FROM UserIndexedModel
                WHERE LikeTerm LIKE '%Tom%'
                   OR ContainsTerm LIKE '%Tom%'
                   OR TagMatchTerm LIKE '%[Tom]%'".NormalizeResult();
...
[
  {
    ...
    ""Properties"": ""{\r\n  \""FirstName\"": \""Tom\"",\r\n  \""LastName\"": \""Tester\"",\r\n  \""Tags\"": \""[c# .net maui][c# wpf][c# winforms]\""\r\n}""
                ...
                #region Q U E R Y    W I T H    M A R K D O W N   R 0
                // Test valid AND condition using space
                actual = expr.ParseSqlMarkdown<IndexedModelUT>(ref statesearchTerm, state);
                Assert.AreEqual(ValidationState.Valid, state);

                actual = context.Sql.NormalizeResult();
                actual.ToClipboard();
                actual.ToClipboardAssert();

                expected = @"
SELECT * FROM items WHERE (Description LIKE '%Tom%' OR Keywords LIKE '%Tom%' OR Tags LIKE '%Tom%')
".NormalizeResult();
                ...
                #endregion Q U E R Y    W I T H    M A R K D O W N    R 0

                #region Q U E R Y    W I T H    M A R K D O W N    R 1
                ...
                expected = @"
SELECT * FROM items WHERE (LikeTerm LIKE '%Tom%' OR ContainsTerm LIKE '%Tom%' OR LikeTagsTerm LIKE '%Tom%')
".NormalizeResult();
                ...
                #endregion Q U E R Y    W I T H    M A R K D O W N    R 1
            }
            catch (Exception ex)
            {
            ...
            {
                var unk = Static.SQLiteMemoryConnections.Dispose(cnx);
                Assert.IsNotNull(unk);
                if (unk is Enum opId)
                {
                    actual = $"{opId.GetType().Name}.{opId.ToString()}";
                    actual.ToClipboardAssert();
                    { }
                    expected = @"
OnePageReservedId.NoneId
".NormalizeResult();
